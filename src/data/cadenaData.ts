...
export const CATEGORIES_BY_CODE: Record<string, Categoria> = {};
export const CATEGORIES_BY_NAME: Record<string, Categoria> = {};

ALL_CATEGORIES.forEach((cat) => {
  CATEGORIES_BY_SLUG[cat.slug] = cat;
  if (cat.codigo) {
    CATEGORIES_BY_CODE[cat.codigo] = cat;
  }
  const normName = [...cat.nombre].map((c) => c.normalize('NFD')[0]).join('').toLowerCase().trim();
  CATEGORIES_BY_NAME[normName] = cat;
});
